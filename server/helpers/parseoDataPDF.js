...
const transformarDataClient = (values) => {  // palabra clave <ind> es indicadores
    let dataToGeneratePdf = {
        infoStudent_Docente: {serYConvivir}, // el objecto studentSelected, tiene, docente, nombres 'alumno', textArea, seccion, grado.
        directora: '',
        coordinadora: '',
        momento: '',
    ...
    // y asi permitir que los tres entren en la primera hoja
    let countInd = 0;
    let conditionMax28Ind = 28;  
    const comprobacion = values.length === 3;
    if( comprobacion ) for (let item of values) countInd += item.indicadores.length;
    ...
    if ( comprobacion && countInd <= conditionMax28Ind) {         
        ...
        const { indCortos, indicadorMenor, indMayorEqual_12 } = filterIndicadoresByLongitud(values);
